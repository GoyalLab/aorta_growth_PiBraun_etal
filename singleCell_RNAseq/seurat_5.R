...
# Define your samples
samples <- c("NU-049", "NU-050", "NU-053", "NU-054")
...
# Loop through and create individual Seurat objects
for (sample in samples) {
  data_dir <- file.path(base_path, sample, "outs", "filtered_feature_bc_matrix")
  expr_matrix <- Read10X(data.dir = data_dir)
  
  seurat_obj <- CreateSeuratObject(counts = expr_matrix, project = sample)
  
  # Add sample name to barcode to keep them unique after merging
...
joint.seurat <- merge(seurat_list[[1]], y = seurat_list[-1], add.cell.ids = samples[-1], project = "Merged")

# (Optional) Save the merged expression matrix if you want to reproduce:
saveRDS(GetAssayData(joint.seurat, assay = "RNA", slot = "counts"), file = "../aggr_matrix/aggr_matrix.rds")
